...
export const CanvasRevealEffect = ({
  ...
  return (
    <div className={cn("relative h-full w-full overflow-hidden bg-transparent", containerClassName)}>
      ...
      {showGradient && (
        <div
          ...
          style={{
            background: `linear-gradient(to top, ${fadeColor} 0%, color-mix(in srgb, ${fadeColor} 72%, transparent) 34%, transparent 84%)`,
          }}
        />
      )}
    </div>
...
const isNumberArray = (value: UniformValue): value is number[] =>
  Array.isArray(value) && value.every((entry) => typeof entry === "number");

const isNumberArrayMatrix = (value: UniformValue): value is number[][] =>
    ...
    for (const uniformName in uniforms) {
      ...
      switch (uniform.type) {
        case "uniform1f":
          ...
          break;
        case "uniform3f":
          if (isNumberArray(uniform.value)) {
        ...
        case "uniform1fv":
          ...
          break;
        case "uniform3fv":
          if (isNumberArrayMatrix(uniform.value)) {
            preparedUniforms[uniformName] = {
              value: uniform.value.map((entry) => new THREE.Vector3().fromArray(entry)),
            };
          }
          break;
        case "uniform2f":
